"""Training script for UNet with YOLO feature distillation."""
...
from src.human_edge_detection.advanced.unet_yolo_feature_distillation import (
    YOLOFeatureDistillationWrapper, YOLODistillationLoss, create_yolo_distillation_model
)
...
def train_epoch(
    ...
    for batch_idx, (images, masks) in enumerate(progress_bar):
        ...
        progress_bar.set_postfix({
            ...
            'feat': f"{loss_dict.get('feature_loss', 0):.4f}",
        ...
        if writer and batch_idx % 10 == 0:
            ...
            writer.add_scalar('Train/Feature_Loss', loss_dict.get('feature_loss', 0), global_step)
...
def evaluate(
    ...
    with torch.no_grad():
        ...
        if test_images is not None:
            test_outputs = model(test_images, extract_features=False)
            student_output_test = test_outputs['student_output']
            teacher_output_test = test_outputs['teacher_output']
    ...
    if writer:
        ...
        writer.add_scalar('Val/Feature_Loss', metrics['feature_loss'], epoch)
...
def get_test_images_by_person_count(dataloader, device='cuda'):
    ...
    (Same implementation as train_distillation_staged.py)
    """
    from pycocotools import mask as mask_utils
    import numpy as np
    ...
    large_roi_img_ids = {
        1: 500716,  # 83.5% ROI coverage
        2: 468954,  # 33.2% ROI coverage
        3: 23899,   # 20.1% ROI coverage
        5: 162732   # Best 5-person image
    }
    ...
    for count in target_counts:
        if selected_samples[count]:
            ...
            # Use fallback if the specific image wasn't found
...
def save_visualization(
    ...
    for i in range(num_samples):
        ...
        student_title = 'Student B0+YOLO (Yellow)'
        ...
        # First apply green for foreground matches
        green_mask = foreground_both & (~mismatch)
        comparison[:, :, 0] = img[:, :, 0] * (1 - alpha_green * green_mask)
        comparison[:, :, 1] = img[:, :, 1] * (1 - alpha_green * green_mask) + \
                              1.0 * alpha_green * green_mask
        comparison[:, :, 2] = img[:, :, 2] * (1 - alpha_green * green_mask)

        # Then apply red for mismatches
        comparison[:, :, 0] = comparison[:, :, 0] * (1 - alpha_red * mismatch) + \
                              1.0 * alpha_red * mismatch
        comparison[:, :, 1] = comparison[:, :, 1] * (1 - alpha_red * mismatch)
        comparison[:, :, 2] = comparison[:, :, 2] * (1 - alpha_red * mismatch)

        axes[i, 3].imshow(comparison)
        axes[i, 3].set_title('Teacher vs Student\n(Green=FG Match, Red=Mismatch)')
        axes[i, 3].axis('off')

    plt.suptitle(f'Epoch {epoch+1:03d} - YOLO Feature Distillation', fontsize=16)
    plt.tight_layout()
...
def export_onnx_model(model: nn.Module, save_dir: Path, device: str):
    """Export model to ONNX format WITHOUT projection layer.

    Args:
        model: Model to export (student model only)
    ...
    try:
        ...
        with torch.no_grad():
            torch.onnx.export(
                ...
                # Use a wrapper to ensure return_features=False
                verbose=False
            )
...
def main():
    parser = argparse.ArgumentParser(description='UNet YOLO Feature Distillation Training')
    ...
    parser.add_argument('--feature_weight', type=float, default=0.5,
                       help='Weight for YOLO feature alignment loss')
    ...
    # Export initial model to ONNX (student only, without projection)
    ...
    for epoch in range(start_epoch, config.training.num_epochs):
        ...
        text_logger.log(f"Epoch {epoch+1:03d} - Train - Loss: {train_metrics['total_loss']:.4f}, "
                       f"KL: {train_metrics['kl_loss']:.4f}, Feature: {train_metrics['feature_loss']:.4f}, "
        ...
        if val_metrics['iou'] > best_iou:
            ...
            # Export best model to ONNX (without projection layer)
            export_onnx_model(model.student, exp_dirs['checkpoints'], args.device)
