...
## ROIAlign/MaskSize 性能分析

RGB Hierarchical UNet V2 Attention アーキテクチャにおける、異なるROIAlignとMaskSizeの組み合わせの性能評価を行いました。

### 性能比較表

| 設定名 | ROI Size | Mask Size | パラメータ数 | GFLOPs | 推論時間 | メモリ使用量 | 相対速度 |
|--------|----------|-----------|-------------|--------|----------|--------------|----------|
| **baseline** | 28 | 56 | 21.7M | 17.19 | 0.5ms | 69.52MB | 1.0x |
| **ROI 112** | | | | | | | |
| r112m224 | 112 | 224 | 21.7M | 274.96 | 8.0ms | 1038.54MB | 0.06x |
| r112m192 | 112 | 192 | 21.7M | 274.96 | 6.51ms | 900.63MB | 0.08x |
| r112m160 | 112 | 160 | 21.7M | 274.96 | 5.26ms | 783.93MB | 0.1x |
| r112m112 | 112 | 112 | 21.7M | 274.96 | 3.8ms | 648.67MB | 0.13x |
| **ROI 96** | | | | | | | |
| r96m192 | 96 | 192 | 21.7M | 202.01 | 5.88ms | 764.31MB | 0.09x |
| r96m160 | 96 | 160 | 21.7M | 202.01 | 4.62ms | 647.62MB | 0.11x |
| r96m112 | 96 | 112 | 21.7M | 202.01 | 3.16ms | 512.36MB | 0.16x |
| **ROI 80** | | | | | | | |
| r80m160 | 80 | 160 | 21.7M | 140.29 | 4.08ms | 532.28MB | 0.12x |
| r80m112 | 80 | 112 | 21.7M | 140.29 | 2.62ms | 397.02MB | 0.19x |
| **ROI 64** | | | | | | | |
| r64m112 | 64 | 112 | 21.7M | 89.78 | 2.18ms | 302.64MB | 0.23x |
| r64m64 | 64 | 64 | 21.7M | 89.78 | 1.24ms | 215.12MB | 0.4x |

### 分析結果

1. **パラメータ数**: すべての設定で同じ（21.7M）- ROIサイズとMaskサイズはモデルの重みには影響しない
2. **計算量 (GFLOPs)**: ROIサイズに比例して増加（28→112で約16倍）
3. **推論時間**: ROIサイズとMaskサイズの両方に依存
4. **メモリ使用量**: 入力解像度の二乗に比例して増加
...
1. **最高品質重視** (研究・評価用)
   - **r112m112**: ROIAlign=112, Mask=112
   - 品質: ★★★★★
   - 速度: 0.13x (7.7倍遅い)
   - メモリ: 648MB
   - 特徴: 最高の解像度で細部まで精密なセグメンテーション

2. **品質優先** (高精度が必要な実用)
   - **r96m112**: ROIAlign=96, Mask=112
   - 品質: ★★★★☆
   - 速度: 0.16x (6.3倍遅い)
   - メモリ: 512MB
   - 特徴: 高品質を維持しつつ、計算コストを抑制

3. **バランス型** (一般的な実用)
   - **r80m112**: ROIAlign=80, Mask=112
   - 品質: ★★★★☆
   - 速度: 0.19x (5.3倍遅い)
   - メモリ: 397MB
   - 特徴: 品質と速度のバランスが良い
...
4. **速度優先** (リアルタイム処理)
   - **r64m64**: ROIAlign=64, Mask=64
   - 品質: ★★★☆☆
   - 速度: 0.4x (2.5倍遅い)
   - メモリ: 215MB
   - 特徴: ベースラインより高品質で、速度低下も許容範囲

### 実装上の考慮事項

1. **バッチサイズの調整**
   - r112m224: バッチサイズを1/16に削減する必要あり
   - r96m112: バッチサイズを1/8に削減
   - r64m64: バッチサイズを1/3程度に削減

2. **Mixed Precision Training**
   - すべての高解像度設定でFP16の使用を推奨
   - メモリ使用量を約半分に削減可能

3. **Gradient Accumulation**
   - 小さいバッチサイズを補うため、勾配累積の使用を推奨
   - 例: バッチサイズ1で8ステップ累積 = 実効バッチサイズ8
...
# 品質重視 (r96m112)
uv run python run_experiments.py \
  --configs rgb_hierarchical_unet_v2_attention_r96m112 \
  --epochs 20 \
  --batch_size 2

# バランス型 (r80m112)  
uv run python run_experiments.py \
  --configs rgb_hierarchical_unet_v2_attention_r80m112 \
  --epochs 20 \
  --batch_size 2

# 速度重視 (r64m64)
uv run python run_experiments.py \
  --configs rgb_hierarchical_unet_v2_attention_r64m64 \
  --epochs 20 \
  --batch_size 2
```
...
- パラメータ数は auxiliary branches を含む全体のモデル
- 推論時間は RTX 3090 基準の概算値
- メモリ使用量は batch_size=1, num_rois=10 での推定値
- 相対速度は baseline (ROI:28, Mask:56) に対する比率
- 実際の性能はハードウェアとデータセットに依存